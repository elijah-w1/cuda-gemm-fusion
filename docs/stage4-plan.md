# 阶段 4 具体计划：PyTorch eager 的融合现状 vs 手写融合

> 背景：torch.compile 需 Triton，Windows+py3.13 无法安装（官方无 wheel、
> 社区源失效），故 阶段 4 改用 torch.profiler 数 kernel 的方式对比。
...
| 手写融合 kernel（阶段 3） | **1** | GEMM+bias+relu 全包 | —— |
...
4. 与 阶段 1 闭环：launch 开销 ~5-8µs 实测 → 融合动机 → 融合收益实测 → 编译器原理
